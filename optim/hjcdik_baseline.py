...
def _morphology_loss(
    ...
    loss = (
        F.relu(se3 - success_eps) + collision_weight * F.relu(collision_margin - critical_dist)
...
def optimize_morphology(
    ...
    try:
        ...
        for update_idx in progress:
            ...
            # Periodic cuRobo validation for logging (after morphology update)
            validation_data = None
            if eval_interval > 0 and update_idx % eval_interval == 0 and logging:
                with torch.no_grad():
                    _, proc_morph_val = _build_morphology_tensors(
                        alpha, lengths.detach(), working_morph.link_radius
                    )
                validation_data = _run_validation(
                    processed_morphology=proc_morph_val[0].detach(),
                    morph=working_morph,
                    task=task,
                    scene=scene,
                    device=device,
                ...
                tqdm.write(
                    ...
                    f"hjcdik-success={pose_success_rate.item() * 100:.2f}%, "
                    f"hjcdik-se3={best_se3_mean.item():.6f}"
                )
        ...
        # --- Final validation ---
        with torch.no_grad():
            final_raw, final_proc = _build_morphology_tensors(
                alpha, lengths.detach(), working_morph.link_radius
            )
        ...
        csv_logger.log_iteration(
            ...
            loss=loss.detach(),
            reachability_probability=pose_success_rate.detach(),
            raw_morphology=final_raw[0].detach(),
            processed_morphology=final_proc[0].detach(),
        ...
        return (
            Morphology(params=final_proc[0].detach(), link_radius=working_morph.link_radius),
            csv_logger.csv_path,
        )
