...
class Decoder(nn.Module):
    def __init__(self, dim_hidden: int = 1792, n_blocks: int = 8, dim_encoding: int = 128):
        ...
        self.model = nn.Sequential(
            ...
            nn.ReLU(),
            *[nn.Sequential(nn.Linear(dim_hidden, dim_hidden), nn.ReLU()) for _ in range(n_blocks)],
            nn.Linear(dim_hidden, 1),
        )

    def forward(self, morph_enc: Float[Tensor, "batch dim_encoding"], pose: Float[Tensor, "batch 9"]) -> Float[Tensor, "batch"]:
