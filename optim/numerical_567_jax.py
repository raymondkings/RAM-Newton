...
try:
    ...
    def _loss_fn(lengths, alpha, task_poses, init_joints, link_radius):
        """
        lengths:    [n_links, 2]
        alpha:      [n_links, 1]
        task_poses: [N, 4, 4]
        ...
        return jnp.mean(dists), (morph, optimal_joints)
    ...
    try:
        ...
        for update_idx in progress_bar:
            ...
            init_joints = jax.random.uniform(
                key, shape=(n_sample, n_links, 1),
                minval=-jnp.pi, maxval=jnp.pi,
            )

            # IK loss + IFT gradient
            (ik_loss, (morph_jax, theta_stars)), ik_grads = _loss_and_grad(
                lengths_jnp, alpha_jnp, poses_batch, init_joints, link_radius
            )
